...
class KgmMediaDataSource(private val kgmFile: File): MediaDataSource() {
    ...
    init {
        ...
        val headerLength: Long = KgmDecoder.calcHeaderSize(buffer, 0)
        ...
        key = buffer.copyOfRange(8, 8 + (0x2c + 1 - 0x1c))
        val key = key
    ...
    override fun readAt(position: Long, buffer: ByteArray?, offset: Int, size: Int): Int {
        ...
        while (i < size) {
            buffer[i] = if (isKgm)
                KgmDecoder.readKgm(index++, buffer[i].toInt(), key).toByte()
            else
                KgmDecoder.readVpr(index++, buffer[i].toInt(), key).toByte()
            i++
        }
        return length
    }
